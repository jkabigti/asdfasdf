namespace Web136.Controllers
{
    ...
    public class LoginController : Controller
    {
        ...
        {
            var request = WebRequest.Create("http://localhost:5419/Api/Authorize/Authenticate?email=" + email + "&password=" + password);
        ...
        public ActionResult Index2(string email, string password)
        {
            ...
            var request = WebRequest.Create("http://localhost:5419/Api/Authorize/Authenticate");
            request.Credentials = CredentialCache.DefaultCredentials;
            request.Method = "POST";
